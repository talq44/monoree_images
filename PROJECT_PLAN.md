...
- 텍스트 파일에 정의된 목록을 기반으로, 무료 내부 모델을 사용하여 이미지를 일괄적으로 자동 생성하는 쉘 스크립트를 개발한다.
- 이 스크립트는 로컬 환경에서 실행되며, 복잡한 설정 없이 몇 가지 입력만으로 이미지 생성을 자동화하는 것을 목표로 한다.

## 2. 스크립트 동작 순서

스크립트를 실행하면 다음 단계의 입력을 순차적으로 받는다.
...
- **설명:** 생성된 이미지를 저장할 폴더의 이름을 입력받는다. (예: `animal/toy3d`)
...
3.  **5단계**에서 선택한 서비스에 맞는 이미지 생성을 실시한다.
4.  이미지를 성공적으로 생성하면, **2단계**에서 생성한 폴더 내에 `{id}.[선택한 확장자]` 형식의 파일 이름으로 저장한다.
5.  하나의 이미지 생성이 완료되면, 다음 이미지 생성을 시작하기 전에 **최소 10초의 딜레이**를 가진다. 이는 과도한 사용 제한을 피하기 위함이다. (예: `sleep 30`)
...
- **결과 로그:** 각 항목에 대한 이미지 생성 성공 또는 실패 여부를 명확히 출력한다. (예: "성공: lion.webp 저장 완료", "실패: tiger - 오류 발생")
...
- **건너뛰기 로직:** 폴더 내에 이미 해당 ID로 생성된 파일(예: `lion.png`)이 존재할 경우, 해당 ID에 대한 이미지 생성을 건너뛰고 다음 항목으로 바로 넘어간다. 이를 통해 작업이 중단되었을 때 처음부터 다시 시작할 필요 없이 중단된 지점부터 효율적으로 작업을 재개할 수 있다.
