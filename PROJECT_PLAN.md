...
스크립트를 실행하면 다음 5단계의 입력을 순차적으로 받는다.
...
- **설명:** Gemini API를 사용하기 위한 인증 토큰(API Key)을 입력받는다.
...
3.  생성된 프롬프트와 **5단계**의 API 토큰을 사용하여 Gemini 이미지 생성 API를 호출한다.
4.  API로부터 이미지 데이터를 성공적으로 받으면, **2단계**에서 생성한 폴더 내에 `{id}.[선택한 확장자]` 형식의 파일 이름으로 이미지를 저장한다.
